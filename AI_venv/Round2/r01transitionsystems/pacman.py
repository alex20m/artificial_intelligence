...
# in Y direction the number of strings, and in the X direction
...
### You can come up with your own names for the different moves
        ...
        #Aux functions, returns the new state if square is not occupied, None if it is
        def MoveForward():
            if d == "N": 
                if not grid.occupied(x, y+1):
                    return PacManState(x,y+1,"N",grid)
            if d == "S":
                if not grid.occupied(x, y-1):
                    return PacManState(x,y-1,"S",grid)
            if d == "E":
                if not grid.occupied(x+1, y):
                    return PacManState(x+1,y,"E",grid)
            if d == "W":
                if not grid.occupied(x-1, y):
                    return PacManState(x-1,y,"W",grid)
            return None
        ...
        if MoveForward() is not None:
            ret_list.append(("MoveForward", MoveForward()))
        if MoveLeft() is not None:
            ret_list.append(("MoveLeft", MoveLeft()))
        if MoveRight() is not None:
            ret_list.append(("MoveRight", MoveRight()))
        if MoveForward() is None and MoveLeft() is None and MoveRight() is None:
            ret_list.append(("TurnAround", TurnAround()))
